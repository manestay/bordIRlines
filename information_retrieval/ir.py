...
output_file_path = 'openai_results.txt'
...
def get_chroma_db(
    ...
    db_list = []  # store Chroma dbs of each language separately
    if from_cache:
        ...
        for language in languages:
            ...
            if not paragraphs:
                ...
                return
            db = Chroma.from_texts(paragraphs, embedding=embedding)
            db_list.append(db)
    
    # If only one language, we don't need to re-embed
    if len(db_list) == 1:
        return db_list[0]
    
    # Since Chroma does not support joining dbs, we will need to re-embed.
    merged_texts = []  # List to hold all raw texts
    for db in db_list:
        # Fetch the raw texts from the current DB
        db_data = db._collection.get(include=['documents'])
        raw_texts = [doc for doc in db_data['documents']]
        merged_texts.extend(raw_texts)  # Collect all raw texts
    merged_texts = list(set(merged_texts)) # Remove duplicate paragraphs
    
    # Create a new Chroma instance using all languages raw texts
    # Chroma's from_texts() keeps all previous documents, so we must manually delete collection as a workaround.
    db_merged = Chroma.from_texts(
        texts=[""],
        embedding=embedding,
        collection_metadata={"hnsw:space": "cosine"}    
    )
    db_merged.delete_collection()
    db_merged = Chroma.from_texts(
        texts=merged_texts, 
        embedding=embedding,
        collection_metadata={"hnsw:space": "cosine"}    
...
def run_ir_system(entity_name, language, query, embedding, from_cache=True, first_n=20, k=10, isLLM=False, relevant_langs=None):
    """
    ...
    print("\nGetting Chroma database")
    db = get_chroma_db(entity_name, languages, embedding, from_cache=from_cache, first_n=first_n, isLLM=isLLM)

    # Perform similarity search with the query
    print(f"Performing Similarity Search over {len(db)} docs\n")
    relevant_docs = db.similarity_search_with_relevance_scores(query, k=k)

    # Extract relevant paragraphs from the retrieved documents, for printing purposes only
    relevant_paragraphs = [tup[0].page_content for tup in relevant_docs]

    print("Top {k} most relevant paragraphs:\n")
...
def run_ir_mmr(entity_name, language, query, embedding, from_cache=True, first_n=20, k=10, isLLM = False):
    db = get_chroma_db(entity_name, language, query, embedding, from_cache=from_cache, k=k, isLLM=isLLM)

    # Perform similarity search with the query
    relevant_docs = db.max_marginal_relevance_search(query, k=k, first_n=k)

    # Extract relevant paragraphs from the retrieved documents
    relevant_paragraphs = [doc[0].page_content for doc in relevant_docs]
...
def save_results(
    ...
    # Query id, Q0, Paragraph ID, RankingNum, Score, Chroma
    # Get hashmap from paragraph to section id
    languages = relevant_langs or [language]
    paragraph_to_id = {}
    for language in languages:
        paragraphs, paragraph_ids = load_paragraphs(entity_name, language, return_para_ids=True, isLLM=isLLM)
        for paragraph, paragraph_id in zip(paragraphs, paragraph_ids):
            paragraph_to_id[paragraph.strip()] = paragraph_id
        
    with open(output_file_path, 'a') as file:  # Open the file in append mode ('a')
        ranking_num = 1
        for doc, score in relevant_docs:
            paragraph_id = paragraph_to_id[doc.page_content.strip()]
            file.write(f'{query_id} Q0 {paragraph_id} {ranking_num} {score} Chroma\n')
            ranking_num += 1
...
def main():
    ...
    for curr_language in languages:
        ...
        for curr_entity_name in territory_names:
            ...
            relevant_docs = run_ir_system(curr_entity_name, curr_language, curr_query, embedding, from_cache, first_n, k=50, isLLM=isLLM, relevant_langs=relevant_langs)
            
            if (store_results):
                save_results(relevant_docs, curr_entity_name, curr_language, curr_query_id, isLLM=isLLM, relevant_langs=relevant_langs)
